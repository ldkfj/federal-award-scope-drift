...
Current local result: 37/37 frontend tests, 28/28 Direct Mode contract tests, production build pass, semantic lint/validation pass, `pip check` pass, and npm audit with zero known vulnerabilities.
...
- Wallet support is limited to EIP-6963-announced OKX Wallet and MetaMask. When OKX also exposes its dedicated injected namespace, that exact provider is preferred over an aggregate browser router. Unknown providers, legacy aggregate injection, WalletConnect, mobile, hardware-wallet, and smart-account flows are not supported or displayed.
